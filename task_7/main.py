"""
Завдання 7: Розширене логування
Розширте попередню програму, щоб додати функцію, яка виконує запис у файл, і включіть логування для цієї функції.
Функція повинна приймати шлях до файлу і дані для запису.
"""
import logging
...
def write_file(f_path, data):
    """
    Записує дані у файл та логуватиме події.
    :param f_path: шлях до файлу
    :param data: дані для запису
    """
    try:
        logging.info(f"Відкриття файлу для запису: {f_path}")
        with open(f_path, 'a') as file:
            file.write(data)
            logging.info(f"Запис даних у файл: {f_path}")
        logging.info(f"Закриття файлу після запису: {f_path}")
    except IOError as e:
        logging.error(f"Помилка вводу/виводу при роботі з файлом {f_path}: {e}")
